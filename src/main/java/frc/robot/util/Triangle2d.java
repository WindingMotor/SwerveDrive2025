...
public class Triangle2d {
    ...
    public boolean isWithinArea(Translation2d point) {
        ...
        return Math.abs(totalArea - getArea()) < 1e-10; // Using small epsilon for floating-point comparison
    }
    ...
    private double distanceToLine(Translation2d point, Translation2d lineStart, Translation2d lineEnd) {
    ...
    public Translation2d[] getEstimatedEdgePoints(double vertices) {
        ...
        int pointsPerSide = (int) vertices / 3;
        int currentPoint = 0;

        // Distribute points along each edge
        for (int side = 0; side < 3; side++) {
            Translation2d start = (side == 0) ? point1 : (side == 1) ? point2 : point3;
            Translation2d end = (side == 0) ? point2 : (side == 1) ? point3 : point1;

            for (int i = 0; i < pointsPerSide && currentPoint < vertices; i++) {
                double t = (double) i / pointsPerSide;
                double x = start.getX() + t * (end.getX() - start.getX());
                double y = start.getY() + t * (end.getY() - start.getY());
                edgePoints[currentPoint++] = new Translation2d(x, y);
            }
        }

        // Fill any remaining points along the first edge
        while (currentPoint < vertices) {
            double t = (double) currentPoint / vertices;
